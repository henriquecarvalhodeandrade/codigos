...
class ControladorOrcamento:
    ...
    def menuOrcamento(self):
        while True:
            ...
            if escolha == '1':
                ...
                while codigo in self.banco_de_dados.orcamentos:
                    print("Código já em uso, tenten novamente.")
                    codigo = input("Digite o código do orçamento: ")
                ...
                cliente_local = input("Digite a localização do cliente: ").lower()
                ...
                cliente = Cliente(cliente_nome, cliente_telefone, cliente_seguradora, cliente_local)
    ...
    def novoOrcamento(self, codigo: str, data_entrada: date, veiculo: Veiculo, servico: Servico, cliente: Cliente):
        ...
        self.banco_de_dados.localizações.addNode(cliente.getLocal())
    ...
    def editarOrcamento(self, codigo: str, nova_data_entrada: datetime.date = None, novo_veiculo_nome: str = None, 
        ...
        if orcamento:
            if nova_data_entrada:
                orcamento.setData(nova_data_entrada)
            if novo_veiculo_nome:
                orcamento.getVeiculo().setNome(novo_veiculo_nome)  # Use getter to access veiculo
            if novo_veiculo_quilometragem is not None:
                orcamento.getVeiculo().setQuilometragem(novo_veiculo_quilometragem)
            if novo_cliente_nome:
                orcamento.getCliente().setNome(novo_cliente_nome)  # Use getter to access cliente
            if novo_cliente_telefone:
                orcamento.getCliente().setTelefone(novo_cliente_telefone)
    ...
    def excluirOrcamento(self, codigo: str):
        if codigo in self.banco_de_dados.orcamentos:
            local = self.banco_de_dados.orcamentos[codigo].getCliente().getLocal()
            self.banco_de_dados.localizações.deleteNode(local)
    ...
    def listarOrcamentos(self):
        if not self.banco_de_dados.orcamentos:
            ...
            counter += 1
            print("--------------------------------------------------")
            self.banco_de_dados.localizações.display()
            print("--------------------------------------------------")
